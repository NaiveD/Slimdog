...
def function_discrete(x1, x2):
    ...
    if (i < 10): # divide the parameter space into 10 discrete values 
        para_space = list(np.arange(-100, 101, 200/10))
    elif (10 <= i < 20): # divide the parameter space into 50 discrete values 
        para_space = list(np.arange(-100, 101, 200/50))
    elif (20 <= i < 30): # divide the parameter space into 100 discrete values 
        para_space = list(np.arange(-100, 101, 200/100))
    elif (30 <= i < 40): # divide the parameter space into 500 discrete values 
        para_space = list(np.arange(-100, 101, 200/500))
    elif (40 <= i < 50): # divide the parameter space into 1000 discrete values 
...
if __name__ == "__main__":
    ...
    file_name = "AGP"
    logger = JSONLogger(path="./AGP/%s.json"%file_name)
